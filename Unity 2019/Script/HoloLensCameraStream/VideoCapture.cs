...
namespace HoloLensCameraStream
{
    ...
    public sealed class VideoCapture
    {
        ...
        public static IEnumerable<Resolution> SupportedResolutions =>
            throw new NotImplementedException(
                "Please use the instance method VideoCapture.GetSupportedResolutions() for now.");
        ...
        /// </summary>
        public bool IsStreaming => _frameReader != null;

        internal SpatialCoordinateSystem worldOrigin { get; private set; }

        public IntPtr WorldOriginPtr
        {
            set => worldOrigin = (SpatialCoordinateSystem) Marshal.GetObjectForIUnknown(value);
        }
        ...
        private VideoCapture(MediaFrameSourceGroup frameSourceGroup, MediaFrameSourceInfo frameSourceInfo,
        ...
        {
            var allFrameSourceGroups =
                await MediaFrameSourceGroup.FindAllAsync(); //Returns IReadOnlyList<MediaFrameSourceGroup>
            var candidateFrameSourceGroups =
                allFrameSourceGroups.Where(group =>
                    group.SourceInfos.Any(IsColorVideo)); //Returns IEnumerable<MediaFrameSourceGroup>
            var selectedFrameSourceGroup =
                candidateFrameSourceGroups.FirstOrDefault(); //Returns a single MediaFrameSourceGroup
            ...
            var selectedFrameSourceInfo =
                selectedFrameSourceGroup.SourceInfos.FirstOrDefault(); //Returns a MediaFrameSourceInfo
        ...
        public IEnumerable<Resolution> GetSupportedResolutions()
        {
            ...
            var allPropertySets = _mediaCapture.VideoDeviceController.GetAvailableMediaStreamProperties(STREAM_TYPE)
                .Select(x => x as VideoEncodingProperties); //Returns IEnumerable<VideoEncodingProperties>
        ...
        public IEnumerable<float> GetSupportedFrameRatesForResolution(Resolution resolution)
        {
            ...
            foreach (var propertySet in allPropertySets)
                ...
                {
                    var frameRate = (float) propertySet.FrameRate.Numerator / (float) propertySet.FrameRate.Denominator;
                    frameRatesDict.Add(frameRate, true);
                }

            //Format resolutions as a list.
            var frameRates = new List<float>();
            foreach (var kvp in frameRatesDict) frameRates.Add(kvp.Key);
        ...
        public async void StartVideoModeAsync(CameraParameters setupParams,
            OnVideoModeStartedCallback onVideoModeStartedCallback)
        {
            ...
            var properties = GetVideoEncodingPropertiesForCameraParams(setupParams);

            // Historical context: https://github.com/VulcanTechnologies/HoloLensCameraStream/issues/6
            if (setupParams.rotateImage180Degrees) properties.Properties.Add(ROTATION_KEY, 180);

            //	gr: taken from here https://forums.hololens.com/discussion/2009/mixedrealitycapture
            IVideoEffectDefinition ved = new VideoMRCSettings(setupParams.EnableHolograms,
                setupParams.EnableVideoStabilization, setupParams.videoStabilizationBufferSize,
        ...
        {
            if (onFrameSampleAcquired == null) throw new ArgumentNullException("onFrameSampleAcquired");
        ...
        public void Dispose()
        {
            if (IsStreaming)
                throw new Exception(
                    "Please make sure StopVideoModeAsync() is called before displosing the VideoCapture object.");
        ...
        {
            if (_mediaCapture != null) throw new Exception("The MediaCapture object has already been created.");
            ...
            await _mediaCapture.InitializeAsync(new MediaCaptureInitializationSettings()
            {
                ...
                MemoryPreference =
                    MediaCaptureMemoryPreference
                        .Cpu, //TODO: Should this be the other option, Auto? GPU is not an option.
        ...
        private VideoEncodingProperties GetVideoEncodingPropertiesForCameraParams(CameraParameters cameraParams)
        {
            var allPropertySets = _mediaCapture.VideoDeviceController.GetAvailableMediaStreamProperties(STREAM_TYPE)
                ...
                .Where((x) =>
                {
                    ...
                    var calculatedFrameRate = (double) x.FrameRate.Numerator / (double) x.FrameRate.Denominator;
    ...
    public class VideoMRCSettings : IVideoEffectDefinition
    {
        ...
        public VideoMRCSettings(bool HologramCompositionEnabled, bool VideoStabilizationEnabled,
            int VideoStabilizationBufferLength, float GlobalOpacityCoefficient)
        {
