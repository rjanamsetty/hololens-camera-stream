...
public static class LocatableCameraUtils
{
    ...
    public static Vector3 PixelCoordToWorldCoord(Matrix4x4 cameraToWorldMatrix, Matrix4x4 projectionMatrix,
        HoloLensCameraStream.Resolution cameraResolution, Vector2 pixelCoordinates)
    {
        pixelCoordinates = ConvertPixelCoordsToScaledCoords(pixelCoordinates, cameraResolution); // -1 to 1 coords

        var focalLengthX = projectionMatrix.GetColumn(0).x;
        var focalLengthY = projectionMatrix.GetColumn(1).y;
        var centerX = projectionMatrix.GetColumn(2).x;
        var centerY = projectionMatrix.GetColumn(2).y;

        // On Microsoft Webpage the centers are normalized 
        var normFactor = projectionMatrix.GetColumn(2).z;
        centerX = centerX / normFactor;
        centerY = centerY / normFactor;

        var dirRay = new Vector3((pixelCoordinates.x - centerX) / focalLengthX,
            (pixelCoordinates.y - centerY) / focalLengthY, 1.0f / normFactor); //Direction is in camera space
        var direction = new Vector3(Vector3.Dot(cameraToWorldMatrix.GetRow(0), dirRay),
            Vector3.Dot(cameraToWorldMatrix.GetRow(1), dirRay), Vector3.Dot(cameraToWorldMatrix.GetRow(2), dirRay));
    ...
    {
        return new Vector3(Vector3.Dot(Vector3.forward, pose.GetRow(0)), Vector3.Dot(Vector3.forward, pose.GetRow(1)),
            Vector3.Dot(Vector3.forward, pose.GetRow(2)));
    }
    ...
    private static Vector2 ConvertPixelCoordsToScaledCoords(Vector2 pixelCoords,
        HoloLensCameraStream.Resolution resolution)
    {
        var halfWidth = (float) resolution.width / 2f;
        var halfHeight = (float) resolution.height / 2f;
